...
public class UploadFinalPDFFileToMinio extends DefaultApplicationPlugin {

    public static String pluginName = "ISTW - DS - Upload FinalPDF To Minio";
    ...
    public Object execute(Map properties) {
        ...
        try {
            ...
            uploadFiletoMinio(processedPdf, recordId + ".pdf", "application/pdf", config);

        } catch (Exception ex) {
            LogUtil.error(UploadFinalPDFFileToMinio.class.getName(), ex, ex.getMessage());
        }
    ...
    protected void uploadFiletoMinio(byte[] bytes, String filename, String contentType, MyConfig config) throws IOException, ServletException {
        try {
            if (bytes.length > 0) {
                try {

                    // Panggil metode untuk mengkonversi byte array ke file
                    File pdfFile = convertBytesToFile(bytes, filename);
                    if (pdfFile != null) {
                        ...
                        String minioFilePath = "digitalSignatureInternal";
                        ...
                        ObjectWriteResponse owr = minioClient.uploadObject(
                                UploadObjectArgs.builder()
                                        ...
                                        .object(minioFilePath + "/" + pdfFile.getName())
    ...
    public String getPropertyOptions() {
        ...
        return AppUtil.readPluginResource(getClassName(), "/properties/uploadFinalPDFFileToMinio.json", (Object[]) new String[]{arr.toString()}, true, "message/uploadFinalPDFFileToMinio");
    }
                ...
                if (signatures.isEmpty()) {
                    ...
                    for (String Signature : selectedSignature) {
                        ...
                        try {
                            ...
                            LogUtil.error(UploadFinalPDFFileToMinio.class.getName(), ex, "Cannot get marker dimension - " + marker.toString());
...
//                            img.setAbsolutePosition(left, bottom - height);
...
//                            img.scaleAbsolute(newWidth, newHeight);
                            ...
                            // Set compression level for image
//                            Image compressionLevel = Image.CompressionLevel.HIGH;
//                            img.setCompressionLevel(compressionLevel);
                ...
                for (int j = 0; j < pages.length(); j++) {
                    ...
                    for (int k = 0; k < objects.length(); k++) {
                        ...
                        if ("labeledRect".equals(object.getString("type"))) {
                            ...
                            BufferedImage qrCodeImage = generateQrCodeImage(url, (int) width, (int) height);

                            int marginRight = -40;
                            int marginBottom = 30;

                            float newLeft = left + (width - qrCodeImage.getWidth()) / 2 - marginRight;
                            float newBottom = pageHeight - top - height - (height - qrCodeImage.getHeight()) / 2 - marginBottom;

                            qrCodeImage = processQrCodeImage(qrCodeImage, (int) width, (int) height);
                            ...
                            img.scaleAbsolute(qrCodeImage.getWidth(), qrCodeImage.getHeight());
                            ...
                            float textX = newLeft + (qrCodeImage.getWidth() - textWidth) / 2;
    ...
    private List<Map> getAnnotationData(String primaryKey) {
        ...
        try {
            ...
            LogUtil.error(UploadFinalPDFFileToMinio.class.getName(), ex, "");
        } finally {
            try {
                if (con != null) {
                    con.close();
    ...
    private List<Map> getQrAnnotationData(String primaryKey) {
        ...
        try {
            ...
            con = ds.getConnection();
            if (!con.isClosed()) {
                PreparedStatement stmt = con.prepareStatement("SELECT qr_anno.* FROM app_fd_ds_qr_pos qr_anno"
                        + " WHERE qr_anno.c_request_id = ?");
                stmt.setObject(1, primaryKey);
                ResultSet rs = stmt.executeQuery();
                ResultSetMetaData meta = rs.getMetaData();
                while (rs.next()) {
    ...
    private void debugMessage(String message) {
        if ("true".equalsIgnoreCase(getPropertyString("debugMode"))) {
            LogUtil.info("" + UploadFinalPDFFileToMinio.class.getName(), "DEBUG MODE: " + message);
        }
    }
    ...
    public BufferedImage convertJsonToImage(String jsonString, int width, int height) {
        Gson gson = new Gson();
        UploadFinalPDFFileToMinio.SignatureLine[] signatureLines = gson.fromJson(jsonString, UploadFinalPDFFileToMinio.SignatureLine[].class);

        double minX = (double) Integer.MAX_VALUE;
        double minY = (double) Integer.MAX_VALUE;
        double maxX = 0;
        double maxY = 0;
        for (UploadFinalPDFFileToMinio.SignatureLine line : signatureLines) {
